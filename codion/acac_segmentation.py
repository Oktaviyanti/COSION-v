# -*- coding: utf-8 -*-
"""
Created on Wed Jul 20 15:43:00 2022
...
from skimage import img_as_float, morphology, img_as_ubyte,segmentation, measure
...
class Segmentation:
    ...
    def __init__(self, base, name):
        if not self.config:
            self.config = settings.MEDIA_ROOT + '/codion'
        
        if not self.filename:
            self.filename = name
        
        self.lokasi = os.path.join(base, name)
        self.information = []

    def ObDetection(self, img, phi):
    ...
    def get_image(self):
        ...
        while i>=0:
            ...
            if beta==0:
                ...
                if Converge:
                    phiShrink,gShrink,initialShrink = self.ObDetection(image,phi)
                    g = 1-gShrink
                    phi = phiShrink
                    beta = 1
            ...
            i += 1
            # usg = f'{i}.png'
            usg = os.path.join(self.config, f'iteration_{i}_{self.filename}')
            usgs.append(usg)
            plt.savefig(usg)
        extent = visual_callback_2d.ax1.get_window_extent().transformed(
            visual_callback_2d.fig.dpi_scale_trans.inverted()
        )

        plt.savefig(
            os.path.join(
                self.config, "output_" + self.filename
            ),
            bbox_inches=extent,
            dpi=300,
            quality=95
        ...
        plt.imsave(
            os.path.join(
                self.config, "img_result_" + self.filename
            ),
            self.imgResult,
            cmap='gray'
        )
